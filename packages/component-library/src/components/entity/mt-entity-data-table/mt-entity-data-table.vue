 ...
 */
import type Repository from "@heyadmin-ag/meteor-admin-sdk/es/data/repository";
import Criteria from "@heyadmin-ag/meteor-admin-sdk/es/data/Criteria";
import { useRepository } from "@heyadmin-ag/meteor-admin-sdk/es/data/composables/useRepository";
import { getRepository } from "@heyadmin-ag/meteor-admin-sdk/es/data/composables/getRepository";
import MtDataTable, {
  type ColumnChanges,
...
const handleItemDelete = async ({ id }: { id: string }) => {
  ...
  if (isInIframe && !props.forceRealModal) {
    const { ui } = await import("@heyadmin-ag/meteor-admin-sdk");
